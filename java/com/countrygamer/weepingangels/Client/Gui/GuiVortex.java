...
public class GuiVortex extends GuiScreen {
	...
	protected void actionPerformed(GuiButton guiB) {
		if (guiB.id == this.dimension.id) {
			...
			for (String key : com.countrygamer.core.common.Core.dimensions.keySet()) {
				dimNames.add(key);
		...
		if (guiB.id == this.saveLocation.id) {
			...
			if (coords != null) {
				...
				this.storeCoords(index, this.nameTextBox.getText(),
						com.countrygamer.core.common.Core.dimensions
		...
		if (guiB.id == this.selectCurrent.id) {
			this.dimension.displayString = com.countrygamer.core.common.Core.dimensions1
		...
		if (guiB.id == this.teleport.id) {
			double[] coords = this.getCoords();
			if (coords != null) {
				int dimID = com.countrygamer.core.common.Core.dimensions

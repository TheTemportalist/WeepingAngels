...
public class EntityWeepingAngel extends EntityCreature {
	...
	protected void applyEntityAttributes() {
		...
		// WeepingAngels.logger.info(ACOptions.maxHealth_Angel + "");
		this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
				WAOptions.maxHealth_Angel);
		...
		this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setBaseValue(this.minSpeed);
		// Attack Damage - default 2.0D - min 0.0D - max Doubt.MAX_VALUE
		this.getAttributeMap().registerAttribute(SharedMonsterAttributes.attackDamage);
		this.getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(
				WAOptions.attackStrength_Angel);
		
	}
	
	protected String getLivingSound() {
		return WeepingAngels.BASE_TEX + "stone";
	}
	
	protected String getHurtSound() {
		return WeepingAngels.BASE_TEX + "light";
	}
	
	protected String getDeathSound() {
		return WeepingAngels.BASE_TEX + "crumble";
	}
	...
	public void onUpdate() {
		...
		if (this.worldObj.getFullBlockLightValue(MathHelper.floor_double(this.posX),
				MathHelper.floor_double(this.posY), MathHelper.floor_double(this.posZ)) > 1.0)
			this.isQuantumLocked = Util.canBeSeenMulti(this.worldObj, this.boundingBox,
		...
		if (this.entityToAttack == null) { // Find an entity to target
			// find closest player
			EntityPlayer player = (EntityPlayer) this.getClosestPlayer(EntityPlayer.class);
			if (player != null) {
	...
	public boolean attackEntityFrom(DamageSource source, float damage) {
		...
		if (source.getSourceOfDamage() instanceof EntityPlayer) {
			...
			if (itemStack != null && itemStack.getItem() == WAItems.sonicScrewdriver) {
				super.attackEntityFrom(source, 0.25F * this.getMaxHealth());
				return true;
			}
			
			if (WAOptions.angelOnlyHurtWithPickaxe) {
				if (itemStack == null)
					canHurt = false;
				else {
					canHurt = itemStack.getItem().canHarvestBlock(Blocks.obsidian, itemStack);
					if (this.worldObj.difficultySetting.getDifficultyId() >= EnumDifficulty.NORMAL
							.getDifficultyId())
						canHurt = canHurt || itemStack.getItem().func_150897_b(Blocks.gold_ore);
				}
			}
			else
				canHurt = true;
	...
	protected void attackEntity(Entity entity, float f) {
		if (entity != null
				&& (!Util.canBeSeenMulti(this.worldObj, this.boundingBox, this.closestPlayerRadius,
						this))) {
			if (entity instanceof EntityPlayer) {
				EntityPlayer entityPlayer = (EntityPlayer) entity;
				if (!entityPlayer.capabilities.isCreativeMode
						&& this.getDistancetoEntityToAttack() <= 2) {
					if (WAOptions.angelCanPoisonTarget
							&& this.rand.nextInt(100) < WAOptions.poisonChance) {
						ExtendedAngelPlayer playerProps = (ExtendedAngelPlayer) ExtendedEntity
								.getExtended(entityPlayer, ExtendedAngelPlayer.class);
						playerProps.setConvertting(true);
						playerProps.setAngelHealth(0.0F);
						playerProps.resetAngelTicks();
						// if (CG_Core.DEBUG)
						// WeepingAngelsMod.log.info("Infected Player");
					}
					else if (WAOptions.angelCanTeleportTarget
							&& this.rand.nextInt(100) < WAOptions.teleportationChance) {
						CoreUtil.teleportPlayer(entityPlayer, 0, WAOptions.teleportationMaxRange,
								entityPlayer.posX, entityPlayer.posZ, true, true);
						this.worldObj.playSoundAtEntity(entityPlayer, WeepingAngels.BASE_TEX
								+ "teleport_activate", 1.0F, 1.0F);
						entity = null;
						// if (CG_Core.DEBUG)
	...
	public boolean getCanSpawnHere() {
		if (WAOptions.spawnRate_Angel == 0) return false;
		...
		if (y <= WAOptions.maxSpawnHeight_Angel) validYLevel = true;
	...
	private Entity getClosestPlayer(Class<?> entity) {
		List<?> list = worldObj.getEntitiesWithinAABB(entity,
				boundingBox.expand(this.closestPlayerRadius, 20D, this.closestPlayerRadius));
		if (!list.isEmpty()) return (Entity) list.get(0);
		return null;
	}
	
	protected void dropRareDrop(int par1) {
		if (WAOptions.addonVortexActive) this.dropItem(WAItems.chrononDust, 1);
	}
	...
	private boolean isEntityFacing(EntityLivingBase entity) {
		...
		if (directionOfEntity == 0 && thisZ > entZ) return true;
		if (directionOfEntity == 1 && thisX < entX) return true;
		if (directionOfEntity == 2 && thisZ < entZ) return true;
		if (directionOfEntity == 3 && thisX > entX) return true;

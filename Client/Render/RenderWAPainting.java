...
public class RenderWAPainting extends RenderPainting {
	private static final ResourceLocation field_110807_a =
			new ResourceLocation(
					Reference.BASE_TEX,
					"textures/items/statue.png");
	
	public void renderThePainting(EntityPainting entPaint,
			double par2, double par4, double par6, float par8, float par9) {
		if(entPaint instanceof EntityWAPainting) {
			EntityWAPainting angel = (EntityWAPainting)entPaint;
			angel.art = WeepingAngelsMod.waa;
			if(angel.art != null) {
				GL11.glPushMatrix();
				GL11.glTranslatef((float)par2, (float)par4, (float)par6);
				GL11.glRotatef(par8, 0.0F, 1.0F, 0.0F);
				GL11.glEnable(GL12.GL_RESCALE_NORMAL);
				this.bindEntityTexture(angel);
				EnumArt enumart = angel.art;//WeepingAngelsMod.waa;//entPaint.art;
				float f2 = 0.0625F;
				GL11.glScalef(f2, f2, f2);
				this.func_77010_a(angel, enumart.sizeX, enumart.sizeY, enumart.offsetX, enumart.offsetY);

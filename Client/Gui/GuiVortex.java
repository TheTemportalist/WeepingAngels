...
public class GuiVortex extends GuiScreen {
	...
	public static final ResourceLocation bkgd = new ResourceLocation(
			Reference.MOD_ID, "textures/gui/" + WeepingAngelsMod.vortexManName
					+ ".png");
	...
	public void initGui() {
		...
		this.indexTextBox = new GuiTextField(this.fontRendererObj,
				this.leftOfGui + (int) (0.25 * this.xSize), this.topOfGui
						+ (int) (0.12 * this.ySize), 30, 12);
		this.setupTextField(this.indexTextBox);

		this.nameTextBox = new GuiTextField(this.fontRendererObj,
				this.leftOfGui + (int) (0.25 * this.xSize), this.topOfGui
						+ (int) (0.21 * this.ySize), 30, 12);
		this.setupTextField(this.nameTextBox);

		width = 70;
		this.newXTextBox = new GuiTextField(this.fontRendererObj, xTextFields,
				yTextFields + 0, width, 12);
		this.setupTextField(this.newXTextBox);

		this.newYTextBox = new GuiTextField(this.fontRendererObj, xTextFields,
				yTextFields + 12, width, 12);
		this.setupTextField(this.newYTextBox);

		this.newZTextBox = new GuiTextField(this.fontRendererObj, xTextFields,
	...
	protected void actionPerformed(GuiButton guiB) {
		if (guiB.id == this.dimension.id) {
			...
			for (String key : com.countrygamer.countrygamer_core.Core.dimensions
					.keySet()) {
				dimNames.add(key);
		...
		if (guiB.id == this.teleport.id) {
			...
			if (coords != null) {
				...
				this.mc.displayGuiScreen(null);
				PacketTeleport packet = new PacketTeleport(dimID, coords,
						false, false);

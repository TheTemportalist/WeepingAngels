...
public class EntityWeepingAngel_OLD extends EntityMob
{
	...
	public EntityWeepingAngel_OLD(World world) {
		super(world);
	...
	private boolean checkForOtherAngels()
	{
		...
		for(int j = 0; j < list.size(); j++)
		{
			...
			if(entity1 instanceof EntityWeepingAngel_OLD)
			{
				EntityWeepingAngel_OLD entityweepingangel = (EntityWeepingAngel_OLD)entity1;
	...
	public boolean angelSeeAngel(EntityWeepingAngel_OLD entity)
	{
		if(worldObj.getFullBlockLightValue(MathHelper.floor_double(posX), MathHelper.floor_double(posY), MathHelper.floor_double(posZ)) < 1)
		{
			return false;
